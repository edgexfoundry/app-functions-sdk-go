...
const (
	serviceKeyPlaceholder = "<serviceKey>"
...
func (tm *pgTableManager) RunScripts(ctx context.Context) error {
	...
	if err = tm.executeSqlFilesDir(ctx, idempotentDBScriptsDir); err != nil {
...
func (tm *pgTableManager) applyMigration(ctx context.Context, dbVersion semver.Version) error {
	...
	for _, version := range sortedVersionDirs {
		...
		err = tm.executeSqlFilesDir(ctx, fmt.Sprintf("%s/%s", versionMigrationDir, version.String()))
		if err != nil {
...
func (tm *pgTableManager) executeSqlFilesDir(ctx context.Context, scriptsDir string) error {
	if len(scriptsDir) == 0 {
	...
	sqlFiles, err := getSortedSqlFileNames(tm.sqlFiles, scriptsDir)
	if err != nil {
		return err
	}

	// begin a transaction to execute all sql files under the same directory
	tx, err := tm.connPool.Begin(ctx)
	if err != nil {
	...
	for _, sqlFile := range sqlFiles {
		if err = tm.executeSqlFile(ctx, tx, sqlFile); err != nil {
			return err
		}
	}
...
func (tm *pgTableManager) executeSqlFile(ctx context.Context, tx pgx.Tx, sqlFilePath string) error {
	// read sql file content
	sqlContent, err := tm.sqlFiles.ReadFile(sqlFilePath)
	if err != nil {
